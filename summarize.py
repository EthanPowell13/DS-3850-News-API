...
def summarize_article(
    ...
    """
    prompt = (
        f"Summarize the following news article for a quick email newsletter. "
        f"Title: {title}\nURL: {url}\n\n"
        f"Article content:\n{content}\n\n"
        "Provide 2–3 bullet points that capture the main points clearly and concisely."
        "Include the article title and a brief description before your summary"
    )
...
if __name__ == "__main__":
    ...
    articles = fetch_articles("technology", max_results=1)
    art = articles[0]
    sample = summarize_article(
        title=art["title"],
        url=art["url"],
